      ...
      switch (msg.type) {
        ...
        case CLIENT_MSG_BALL_POSITION: {
          ...
          printf("Ball from client: %d position: (%.2f, %.2f)\n",
                 con->client_id, msg.data.position.x, msg.data.position.y);
...
void sr_start_udp_listen(UDPServer* server) {
  ...
  while (1) {
    ...
    ssize_t recv_len = recvfrom(server->fd, buffer, sizeof(ClientMessage), 0,
    ...
    if (recv_len == sizeof(ClientMessage)) {
      ...
      if (msg->type == CLIENT_MSG_REGISTER) {
        int client_id = sr_add_udp_client(server, client_addr, client_len);

        if (client_id >= 0) {
          printf("New client registered with ID %d from %s:%d\n", client_id,
                 inet_ntoa(client_addr.sin_addr), ntohs(client_addr.sin_port));

          ServerMessage confirm_msg = {.type = SERVER_MSG_REGISTER_CONFIRM,
                                       .client_id = client_id,
                                       .timestamp = time(NULL)};

          sr_send_udp_message_to_client(server, client_id, &confirm_msg);
          ...
          if (client_id == 0) {
            ...
            printf("Sending IS_MAIN message to client %d (first player)\n",
                   client_id);
            sr_send_udp_message_to_client(server, client_id, &is_main_msg);
          }

          ServerMessage new_player_msg = {.type = SERVER_MSG_PLAYER_JOINED,
                                          .client_id = client_id,
                                          .timestamp = time(NULL)};
          sr_send_udp_message_to_all_except(server, client_id, &new_player_msg);
        } else {
          printf("Failed to register new client - server full\n");

          ServerMessage reject_msg = {.type = SERVER_MSG_REGISTER_CONFIRM,
                                      .client_id = -1,
                                      .timestamp = time(NULL)};

          sendto(server->fd, &reject_msg, sizeof(ServerMessage), 0,
                 (struct sockaddr*)&client_addr, client_len);
        }
      } else {
...
void sr_handle_udp_message(UDPServer* server, int client_id,
                           ClientMessage* msg) {
  switch (msg->type) {
    case CLIENT_MSG_PADDLE_POSITION: {
      printf("Client %d position: (%.2f, %.2f)\n", client_id,
      ...
      ServerMessage pos_broadcast = {.type = SERVER_MSG_PADDLE_POSITION_UPDATE,
                                     .client_id = client_id,
                                     .timestamp = time(NULL),
                                     .data.position = msg->data.position};
      sr_send_udp_message_to_all_except(server, client_id, &pos_broadcast);
      break;
    }
    case CLIENT_MSG_BALL_POSITION: {
      if (client_id != 0) {
        printf("Warning: Non-main client %d tried to send ball position\n",
               client_id);
        break;
      }

      printf("Ball from client %d: (%.2f, %.2f)\n", client_id,
             msg->data.position.x, msg->data.position.y);
...
int sr_add_udp_client(UDPServer* server, struct sockaddr_in addr,
                      socklen_t addr_len) {
  if (server->client_count >= MAX_PLAYERS) {
...
void sr_check_client_timeouts(UDPServer* server) {
  ...
  for (int i = 0; i < MAX_PLAYERS; i++) {
    UDPClientConnection* con = &server->clients[i];
    if (con->active && (current_time - con->last_seen) > DISCONNECT_TIMEOUT) {
...
void sr_send_udp_message_to_all(UDPServer* server,
                                const ServerMessage* message) {
  pthread_mutex_lock(&server->clients_mutex);

  for (int i = 0; i < MAX_PLAYERS; i++) {
    if (server->clients[i].active) {
      sendto(server->fd, message, sizeof(ServerMessage), 0,
...
void sr_send_udp_message_to_all_except(UDPServer* server, int except_client_id,
                                       const ServerMessage* message) {
  pthread_mutex_lock(&server->clients_mutex);
...
void sr_send_udp_message_to_client(UDPServer* server, int client_id,
                                   const ServerMessage* message) {
  pthread_mutex_lock(&server->clients_mutex);
...
int sr_udp_client_connect(UDPClient* client, const char* server_ip,
                          unsigned short port) {
  if (!server_ip) {
    printf("ip is NULL\n");
    return -1;
  ...
  printf("UDP client created, ready to send to server %s:%d\n", server_ip,
         port);
    ...
    if (sr_receive_udp_server_message(client, &response) == 0) {
      ...
      if (response.type == SERVER_MSG_REGISTER_CONFIRM) {
        if (response.client_id >= 0) {
          ...
          client->registered = 1;
          client->is_main = 0;
          printf("Successfully registered with server, client ID: %d\n",
                 client->client_id);
...
int sr_receive_udp_server_message(UDPClient* client, ServerMessage* msg) {
  ...
  ssize_t bytes_read = recvfrom(client->server_fd, msg, sizeof(ServerMessage),
...
int sr_send_udp_message_to_server(UDPClient* client, const ClientMessage* msg) {
  ...
  ssize_t bytes_sent =
      sendto(client->server_fd, msg, sizeof(ClientMessage), 0,
