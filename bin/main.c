...
static void ProcessInputSystem(ecs_iter_t* it);

static void MoveEnemySystem(ecs_iter_t* it);

static void UpdateBallFromNetwork(ecs_iter_t* it);
...
int main(void) {
  ...
  ECS_SYSTEM(world, BallWallCollisions, EcsOnUpdate,
             Position, [inout] BallMovement, [in] Collider,

             Ball);
  ECS_SYSTEM(world, BallPaddleSoundOnCollission,
             EcsOnUpdate, [in] Position, [in] Collider, [in] GameSound, Ball);
  ECS_SYSTEM(world, BallWallSoundOnCollission,
             EcsOnUpdate, [in] Position, [in] Collider, [in] GameSound, Ball);
  ...
  ecs_entity_t upper_wall = ecs_insert(
      world, ecs_value(Position, {.x = 0, .y = 0}),
      ecs_value(Collider,
  ...
  while (!WindowShouldClose()) {
    ...
    if (client->is_main) {
      ...
      ecs_run(world, ecs_id(BallPaddleSoundOnCollission), GetFrameTime(), NULL);
      ecs_run(world, ecs_id(BallWallSoundOnCollission), GetFrameTime(), NULL);
